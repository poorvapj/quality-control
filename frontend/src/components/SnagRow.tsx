...
import { refLabel, snagTarget, rccItemLabel } from "../shared/rules";
  ...
  return (
    <div className={"qitem" + (s.severity === "Critical" ? " alert" : " warn")} onClick={(e) => { e.stopPropagation(); openDrawer({ kind: "snag", id: s.id }); }}>
      <div className="qitem-main">
        ...
        <div className="qitem-sub">{snagTarget(data, s)} · {s.itemId ? rccItemLabel(data, s.itemId) : refLabel(data, "stages", s.stageId)} · raised by {refLabel(data, "users", s.raisedBy)} {ago(s.raisedAt)}</div>
      </div>
